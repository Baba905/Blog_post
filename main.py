...
class User(db.Model, UserMixin):
    ...
    comments = relationship("Comment", back_populates="commenter_author")

class Comment(db.Model):
    __tablename__ = "comments"
    id = db.Column(db.Integer, primary_key=True)
    text = db.Column(db.String(), nullable=True)
    commenter_author = relationship("User",back_populates="comments")
    author_id = db.Column(db.Integer,db.ForeignKey("users.id"))
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            flash('That email is not registred in our database')
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    list_comments = db.session.execute(db.select(Comment).where(Comment.blog_post_id== post_id)).scalars()
    ...
    return render_template("post.html", post=requested_post, form =comment_form, comments = list_comments)
